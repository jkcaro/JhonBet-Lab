...
def _sec(titulo: str, opcional: bool = False) -> None:
    """Cabecera de sección compacta para el formulario."""
    sfx = ' <span style="font-weight:400;opacity:.5;font-size:8px;">(opcional)</span>' if opcional else ""
    st.markdown(
        f'<div style="font-size:9px;font-weight:700;color:#8eb0cc;text-transform:uppercase;'
        f'letter-spacing:1.5px;margin:10px 0 3px 0;padding-bottom:3px;'
        f'border-bottom:1px solid #1a2540;">{titulo}{sfx}</div>',
        unsafe_allow_html=True,
    )


@st.dialog("➕ Agregar Partido Manual", width="large")
def _dialogo_partido_manual() -> None:
    """Diálogo modal para añadir un partido manualmente."""

    # ── CSS del diálogo — tema SCADA ────────────────────────────────────────
    st.markdown("""
<style>
...
div[data-testid="stDialog"] > div {
    ...
    box-shadow: 0 24px 60px rgba(0,0,0,.7), 0 0 0 1px rgba(0,255,136,.08) !important;
}
...
.stSelectbox > div > div > div { padding: 3px 10px !important; color: #c8d8e8 !important; }
.stSelectbox > div > div:focus-within { border-color: #00ff88 !important; }

/* ── Labels ── */
div[data-testid="stWidgetLabel"] p,
.stTextInput label, .stNumberInput label, .stSelectbox label {
    font-size: 11px !important;
    color: #8eb0cc !important;
...
div[data-testid="stDialog"] .stButton > button:hover {
    background: #254a78 !important;
    border-color: #8eb0cc !important;
...
/* ── Spinner ── */
div[data-testid="stDialog"] .stSpinner p { font-size: 12px !important; }

/* ── Título nativo del diálogo ── */
...
div[data-testid="stDialog"] [data-testid="stHeading"] {
    ...
    # ── Banner de cabecera del modal ─────────────────────────────────────────
    st.markdown(
        '<div style="background:#080c14;'
        'border:1px solid #1a2540;border-radius:8px;padding:10px 14px;margin-bottom:12px;'
        'display:flex;align-items:center;gap:10px;">'
        '<span style="font-size:18px;">⚽</span>'
        '<div>'
        '<div style="font-size:12px;font-weight:700;color:#8eb0cc;letter-spacing:.3px;">'
        'Nuevo partido manual</div>'
        '<div style="font-size:10px;color:#5a7a9a;margin-top:1px;">'
    ...
    with col_b:
        buscar_click = st.button("🔍 Buscar", key="btn_buscar_equipo", use_container_width=True)

    if buscar_click:
        if termino.strip():
            from modules.odds_api import buscar_por_equipo
            with st.spinner("Buscando partidos en The Odds API..."):
                resultados = buscar_por_equipo(termino.strip())
            st.session_state["pm_resultados"] = resultados
            for k in ("pm_match_sel", "pm_prefill"):
                st.session_state.pop(k, None)
        else:
            st.warning("Escribe el nombre de un equipo antes de buscar.")

    resultados = st.session_state.get("pm_resultados")
    if resultados is not None:
        if not resultados:
            st.caption("Sin resultados para ese equipo hoy.")
        else:
            opciones = ["-- Selecciona un partido --"] + [r["label"] for r in resultados]
    ...
    with col_xv:
        xg_visitante = st.number_input(
            "xG Visit.", min_value=0.1, max_value=5.0, value=1.2, step=0.1, key="pm_xgv",
        )

    # ── DATOS ADICIONALES (opcionales, colapsados) ───────────────────────────
    with st.expander("➕ Datos adicionales (opcional)", expanded=False):
        ...
        with col_bl5:
            btts_local_5 = st.number_input(
                "BTTS local", min_value=0, max_value=5, value=None, step=1,
                key="pm_btts_l5",
                help="Partidos de los últimos 5 con Ambos Marcan — local",
            )
        with col_bv5:
            btts_visit_5 = st.number_input(
                "BTTS visit.", min_value=0, max_value=5, value=None, step=1,
                key="pm_btts_v5",
        ...
        with col_ev:
            elo_visit = st.number_input(
                "ELO visit.", min_value=0, max_value=3000, value=None, step=1,
                key="pm_elo_v",
        ...
        with col_mot:
            motivacion = st.selectbox(
                ...
                ["No sé / Sin datos", "Sí — ascenso/descenso", "Parcialmente", "No — sin motivación"],
                key="pm_motivacion",
            )
        with col_ult:
        ...
        if "sin motivación" in motivacion.lower():
            st.markdown(
                '<div style="font-size:10px;color:#f59e0b;background:rgba(245,158,11,.07);'
                'border:1px solid rgba(245,158,11,.25);border-radius:5px;padding:5px 10px;margin-top:4px;">'
                '⚠️ Partido sin motivación — se restará 1 punto del sistema de puntuación</div>',
                unsafe_allow_html=True,
            )
        if ultimo_partido == "Sí":
            st.markdown(
                '<div style="font-size:10px;color:#f59e0b;background:rgba(245,158,11,.07);'
                'border:1px solid rgba(245,158,11,.25);border-radius:5px;padding:5px 10px;margin-top:4px;">'
    ...
    # variables con valor por defecto cuando el expander está cerrado
    ...
    # ── BOTONES DE ACCIÓN ─────────────────────────────────────────────────────
    st.markdown(
        '<hr style="border-color:#1e1e38;margin:12px 0 10px">',
        unsafe_allow_html=True,
    )
    col_guardar, col_cancelar = st.columns([5, 1])
    with col_guardar:
        guardar = st.button(
            "💾  Guardar partido", key="btn_agregar_manual", use_container_width=True,
        )
    with col_cancelar:
        if st.button("✖", key="btn_cancelar_manual", use_container_width=True,
                     help="Cerrar sin guardar"):
            st.rerun()
